...
/**
 * Test to verify that the kafka-plugin can be loaded correctly by nrtSearch.
 * This is a simpler test that doesn't require Docker/Testcontainers.
 */
public class PluginLoadingE2ETest extends NrtsearchTest {
  ...
  private void setupPluginDirectory(Path pluginSearchPath) throws Exception {
    ...
    if (!metadataFile.toFile().exists()) {
      throw new RuntimeException("Plugin setup failed - metadata file not found at: " + metadataFile);
    }
  ...
  private void buildPluginDistribution(String projectRoot) throws Exception {
    ...
    pb.directory(new java.io.File(projectRoot).getParentFile()); // Go up to nrtsearch-ingestion-plugins
  ...
  private void extractPluginDistribution(String tarPath, Path targetDir) throws Exception {
    ...
    if (extractedDir.toFile().exists()) {
      ...
      if (!extractedDir.toFile().renameTo(finalDir.toFile())) {
        throw new RuntimeException("Failed to rename plugin directory from " + extractedDir + " to " + finalDir);
      }
      LOGGER.info("Renamed plugin directory: {} -> {}", extractedDir.getFileName(), finalDir.getFileName());
    } else {
